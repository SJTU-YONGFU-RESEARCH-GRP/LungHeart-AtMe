...
class AugmentedConv(nn.Module):
    ...
    def compute_flat_qkv(self, x, dk, dv, Nh, q_new=None):
        qkv = self.qkv_conv(x)
        N, _, H, W = qkv.size()
        ...
        dkh = dk // Nh
        q_new = q * (dkh ** -0.5)
        flat_q = torch.reshape(q_new, (N, Nh, dk // Nh, H * W))
    ...
    def rel_to_abs(self, x):
        ...
        final_x0 = torch.reshape(flat_x_padded, (B, Nh, L + 1, 2 * L - 1))
        final_x = final_x0[:, :, :L, L - 1:]
        return final_x
